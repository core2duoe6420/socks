...
class Select(object):
    ...
    def add_sock(self, sock, **kwargs):
        ...
        self._log.info("add socket, fd=%d" % (sock.fileno()))

    def del_sock(self, sock):
        if sock in self._sock_set:
            self._log.info("del socket, fd=%d" % (sock.fileno()))
    ...
    def modify_sock(self, sock, event, handler):
        if sock not in self._sock_set:
            self._log.warning("sock %s does not exist in Select._sock_set" % (str(sock)))
            return
    ...
    def run(self):
        while True:
            try:
                ...
                self._log.warning("Too many sockets in select, error on last 1/4 sockets")
                err_list = self._sock_lists[EVT_ERROR]
                sock_num = len(err_list)
                for i in xrange(sock_num - 1, sock_num * 3 / 4, -1):
                    sock = err_list[i]
                    self._log.event("%s, fd=%d" % ("on_clean", sock.fileno()))
                    self._sock_set[sock][EVT_ERROR](sock)
            except:
                raise
...
class Epoll(object):
    ...
    def del_sock(self, sock):
        ...
        self._log.info("del socket, fd=%d" % fd)
        ...
        while True:
            ...
            for fd, event in events:
                ...
                if event & select.EPOLLIN:
                    self._log.event("%s, fd=%d" % (EVT_READ, fd))
                    if sock_info[EVT_READ] is not None:
                        sock_info[EVT_READ](sock)
                    else:
                        self._log.warning("socket %d on_read callback is None" % fd)

                if fd in self._sock_set:
                    if event & select.EPOLLOUT:
                        self._log.event("%s, fd=%d" % (EVT_WRITE, fd))
                ...
                if fd in self._sock_set:
                    if event & select.EPOLLERR or event & select.EPOLLHUP:
                        self._log.event("%s, fd=%d" % (EVT_ERROR, fd))
